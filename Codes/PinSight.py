...
from manual_profile_editor import ManualProfileEditor
import requests
# --- MATPLOTLIB SETUP ---
import matplotlib
    ...
    for c in cnts:
        ...
        if 50 < area < 25000:
            ...
            if m["m00"]:
                ...
                blobs.append({'x': cx, 'y': y, 'h': h})
    return blobs,baseline_y
# ============================================================
...
def extract_pins(
    ...
    blobs,baseline_y = detect_blobs(warped,dst_h)
    ...
    # --- REDEFINE height as true pin length ---
    for p in pins:
        p['h'] = baseline_y - p['y']
...
def calculate_all_metrics(pins, pitch_cm, do_leveling=True, units="px"):

    # --------------------------------------------------
    # 1. Sort pins and build physical coordinates
    # --------------------------------------------------
    pins_sorted = sorted(pins, key=lambda p: p['x'])
    n = len(pins_sorted)

    if n < 3:
        raise ValueError("Not enough points for roughness analysis")

    if units == "px":
        board_x_cm = pitch_cm[0]   # we will pass tuple (X, Z)
        board_z_cm = pitch_cm[1]

        W_px = 1200
        H_px = 400

        scale_x = board_x_cm / W_px
        scale_z = board_z_cm / H_px

        x = np.array([p['x'] for p in pins_sorted]) * scale_x
        z_raw = np.array([p['h'] for p in pins_sorted]) * scale_z

    elif units == "cm":
        # Manual pipeline already in cm
        x = np.array([p['x'] for p in pins_sorted])
        z_raw = np.array([p['h'] for p in pins_sorted])
    ...
    # MATLAB: shift heights to minimum = 0
    z_raw = z_raw - np.min(z_raw)

    # --------------------------------------------------
    # 2. Leveling 
    # --------------------------------------------------
    if do_leveling:
        pente, origine = np.polyfit(x, z_raw, 1)
        z_used = z_raw - (pente * x + origine)
    else:
        z_used = z_raw.copy()
    ...
    # 3. RMS height 
    # --------------------------------------------------
    z_centered = z_used - np.mean(z_used)
    h_rms = np.sqrt(np.mean(z_centered ** 2))
    ...
    for h in range(max_lag):
        num = np.sum(z_centered[:n - h] * z_centered[h:])
        rho.append(num / denom)

    rho = np.array(rho)

    # --------------------------------------------------
    # 5. Correlation length (1/e crossing)  : for h=0 rho=1, we look for the first h where rho < 1/e
    # --------------------------------------------------
    target = 1.0 / np.e
    l_cm = 0.0
    ...
    for i in range(1, len(rho)):
        if rho[i] < target:
            ...
            frac_lag = (i - 1) + (target - y1) / (y2 - y1)
            mean_dx = np.mean(np.diff(x))
            l_cm = frac_lag * mean_dx
            break
    ...
    # --------------------------------------------------
    # 7. Chain index (tortuosity)
    # --------------------------------------------------
    L1 = x[-1] - x[0]
    dx = np.diff(x)
    dz = np.diff(z_used)
    L2 = np.sum(np.sqrt(dx ** 2 + dz ** 2))
    ic_val = (1.0 - L1 / L2) * 100.0 if L2 > 0 else 0.0

    # --------------------------------------------------
    # 8. Fractal dimension (semivariogram)
    # --------------------------------------------------
    sv_h = []
    sv_g = []

    max_lag_sv = max(3, n // 3)

    for k in range(1, max_lag_sv):
        mean_dx = np.mean(np.diff(x))
        h = k * mean_dx #physical lag
    ...
    if len(sv_h) > 2:
        log_h = np.log(sv_h)
        log_g = np.log(sv_g)
        slope, intercept, _, _, _ = linregress(log_h, log_g)
        df_val = 2.0 - slope / 2.0
    ...
    return {
        ...
        'plot_data': {
            ...
            'z_plot': z_plot,
            'z_orig': z_raw,
            'lags': np.arange(len(rho)) * np.mean(np.diff(x)),
            'rho': rho,
            'sv_h': sv_h,
            'sv_g': sv_g,
            'slope': slope,
...
class VisualizationWidget(QWidget):
    ...
    def plot_metrics(self, data):
        ...
        if data['sv_h']:
            ...
            ax3.loglog(x_fit, y_fit, 'r-', label=rf'$y = {data["slope"]:.2f}x {data["intercept"]:+.2f}$')
    ...
    def run(self):
        ...
        for i, p in enumerate(self.paths):
            ...
            if data[-1]:
                ...
                continue
            if len(data[1])>1:
                metrics = calculate_all_metrics(data[1], self.pitch,self.do_leveling, units="px")
                metrics['viz'] = data[:-1]
                metrics['path'] = p
                metrics['pins'] = data[1]
                ...
                self.invalid_image.emit(
                    os.path.basename(p),
                    p,
                    "Not enough pins detected to compute metrics."
                )
                self.progress.emit(i + 1)
                continue
            self.progress.emit(i + 1)
...
class PinApp(QMainWindow):
    def __init__(self):
        ...
        self.resize(1600, 1000) # Slightly taller initial window
    ...
    def _init_ui(self):
        ...
        board_box.setSpacing(8)

        lbl_board = QLabel("BOARD DIMENSIONS (CM)")
        lbl_board.setObjectName("PitchLabel")
        board_box.addWidget(lbl_board)

        # ===== Row 1: Horizontal length =====
        row_x = QHBoxLayout()
        label_x = QLabel("Horizontal length (X):")
        label_x.setStyleSheet("color: black;")
        label_x.setFixedWidth(160)  # keeps alignment clean
        self.board_x_in = QLineEdit("113")

        row_x.addWidget(label_x)
        row_x.addWidget(self.board_x_in)

        board_box.addLayout(row_x)

        # ===== Row 2: Vertical length =====
        row_z = QHBoxLayout()
        label_z = QLabel("Vertical length (Z):")
        label_z.setStyleSheet("color: black;")
        label_z.setFixedWidth(160)
        self.board_z_in = QLineEdit("53")

        row_z.addWidget(label_z)
        row_z.addWidget(self.board_z_in)

        board_box.addLayout(row_z)

        s_lay.addLayout(board_box)

        self.board_x_in.editingFinished.connect(self.recompute_all_metrics)
        self.board_z_in.editingFinished.connect(self.recompute_all_metrics)
        ...
        # Actions
        btn_f = QPushButton("Add Images")
        btn_f.setObjectName("PrimaryAction")

        btn_manual = QPushButton("Manual Profile")
        btn_manual.setToolTip("Select image and define surface profile manually")

        btn_d = QPushButton("Add Folder")

        btn_f.clicked.connect(self.load_files)
        btn_d.clicked.connect(self.load_folder)
        btn_manual.clicked.connect(self.open_manual_from_file)

        btn_lay = QHBoxLayout()
        btn_lay.addWidget(btn_f)
        btn_lay.addWidget(btn_manual)
        btn_lay.addWidget(btn_d)
        ...
        self.img_view.setMinimumHeight(400)
        ...
        self.btn_edit_pins.clicked.connect(self.on_edit_button_clicked)
        ...
        self.details_table.setHorizontalHeaderLabels(["Image", "Source", "RMS (cm)", "L (cm)", "H_max (cm)", "IC (%)", "DF"])
        self.details_table.horizontalHeader().setSectionResizeMode(0, QHeaderView.Stretch)
        for c in range(1, 7): self.details_table.horizontalHeader().setSectionResizeMode(c, QHeaderView.ResizeToContents)
        ...
        self.details_table.cellDoubleClicked.connect(self.table_row_double_clicked)
    ...
    def on_edit_button_clicked(self):
        ...
        # ---------- MANUAL ----------
        if res.get("source") == "manual":
            # Force button to behave like a normal button
            self.btn_edit_pins.setChecked(False)
            self.btn_edit_pins.setText("✏ Edit pins")
            self.btn_edit_pins.setToolTip("Edit pins")

            self.open_manual_editor_for_row(self.current_idx)
            return

        # ---------- CV ----------
        if res.get("source") == "cv":
            if not self.cv_editing_active:
                self.enable_cv_editing()
            else:
                self.disable_cv_editing()


    def enable_cv_editing(self):
        self.cv_editing_active = True
        self.img_view.set_edit_mode(True)

        self.btn_edit_pins.setChecked(True)
        self.btn_edit_pins.setText("✔ Finish editing")
        self.btn_edit_pins.setToolTip("Finish pin editing")


    def disable_cv_editing(self):
        self.cv_editing_active = False
        self.img_view.set_edit_mode(False)

        self.btn_edit_pins.setChecked(False)
        self.btn_edit_pins.setText("✏ Edit pins")
        self.btn_edit_pins.setToolTip("Edit pins manually")


    def open_manual_editor_for_row(self, row):
        """
        Open manual editor for a given table row (manual only).
        """
        ...
        if res.get("source") != "manual":
            QMessageBox.information(
                self,
                "Not a manual profile",
                "Only manually digitized profiles can be edited."
            )
            return

        manual_state = res.get("manual_state")
        roi_img = res.get("roi_image")

        if manual_state is None:
            QMessageBox.warning(
                self,
                "Cannot reopen manual editor",
                "This manual profile cannot be reopened because its editor state is missing."
            )
            return
        ...
        if roi_img is None:
            ...
            if img is None:
                QMessageBox.critical(
                    self,
                    "Error",
                    "Could not reload the original image."
                )
                return
            ...
            QMessageBox.information(
                self,
                "ROI not found",
                "The cropped ROI image was not saved.\n"
                "The editor will open using the original image."
            )

        dlg = ManualProfileEditor(
            roi_img,
            initial_state=manual_state,
            parent=self
        )

        if dlg.exec():
            manual = dlg.get_result()

            metrics = calculate_all_metrics(
                manual["pins"],
                (
                    float(self.board_x_in.text()),
                    float(self.board_z_in.text())
                ),
            ...
            res.update({
                "roi_image": manual["warped_image"],
                "viz": (manual["warped_image"], []),
                "pins": manual["pins"],
                "manual_state": manual["editor_state"],
                **metrics
            })
    ...
    def edit_result_for_row(self, row):
        """
        Edit a result depending on its source:
        - MANUAL → open manual editor
        - CV → enable pin editing
        """
        res = self.results[row]
        ...
        # ---------- MANUAL ----------
        if res.get("source") == "manual":
            self.open_manual_editor_for_row(row)
            return
    ...
    def table_row_double_clicked(self, row, col):
        self.edit_result_for_row(row)
    ...
    def open_manual_from_file(self):
        path, _ = QFileDialog.getOpenFileName(
            self,
            "Select image for manual profile",
            "",
            "Images (*.png *.jpg *.jpeg)"
        )
        ...
        existing = [r for r in self.results if r["path"] == path]

        if existing:
            sources = {r.get("source") for r in existing}

            # === Manual → Manual ===
            if "manual" in sources:
                msg = QMessageBox(self)
                msg.setWindowTitle("Manual profile exists")
                msg.setText(
                    "This image already has a manual profile.\n\n"
                    "What would you like to do?"
                )

                btn_resume = msg.addButton("Resume manual editing", QMessageBox.AcceptRole)
                btn_cv = msg.addButton("Process using CV", QMessageBox.ActionRole)
                btn_cancel = msg.addButton("Do nothing", QMessageBox.RejectRole)
                ...
                if clicked == btn_resume:
                    manual_state = next(
                        (r.get("manual_state") for r in existing if r.get("source") == "manual"),
                        None
                    )
                    ...
                    if manual_state is None:
                        QMessageBox.warning(
                            self,
                            "Cannot resume",
                            "The manual editor state is missing or incompatible.\n\n"
                            "The editor will be opened from scratch."
                        )

                    roi_img = next(
                        r.get("roi_image") for r in existing if r.get("source") == "manual"
                    )

                    dlg = ManualProfileEditor(
                        roi_img,                 # 👈 already cropped image
                        initial_state=manual_state,
                        parent=self
                    )
                    ...
                    self.run_batch([path], append=True)
            ...
            if "cv" in sources:
                reply = QMessageBox.question(
                    self,
                    "CV result exists",
                    "This image was already processed automatically.\n\n"
                    "Do you want to digitize it manually?",
                    QMessageBox.Yes | QMessageBox.No,
                    QMessageBox.Yes
                )
    ...
    def open_manual_from_failed_image(self, image_path):
        img = cv2.imread(image_path)
        if img is None:
            QMessageBox.critical(self, "Error", "Could not load image.")
            return
    ...
    def show_invalid_image_popup(self, filename,image_path, reason):
        ...
        manual_btn = msg.addButton("Manual digitization", QMessageBox.ActionRole)
        ok_btn = msg.addButton(QMessageBox.Ok)

        msg.exec()
        
        if msg.clickedButton() == manual_btn:
            self.open_manual_from_failed_image(image_path)
    ...
    def find_result_by_path(self, path):
        for i, r in enumerate(self.results):
            if r["path"] == path:
                return i, r
        return None, None
    def replace_result(self, index, new_entry):
        self.results[index] = new_entry
    ...
    def on_done(self, res):
        ...
        # ---------- 🔐 SOURCE-AWARE DUPLICATE HANDLING (CV SIDE) ----------
        if self.results:
            existing_by_path = {}
            for r0 in self.results:
                existing_by_path.setdefault(r0['path'], []).append(r0)

            filtered = []

            for r in res:
                path = r['path']
                src = r.get("source", "cv")  # always "cv" here
                ...
                existing_sources = {e.get("source", "cv") for e in existing_by_path[path]}

                # === CV → CV ===
                if "cv" in existing_sources:
                    reply = QMessageBox.question(
                        self,
                        "Image already processed",
                        f"The image:\n\n{os.path.basename(path)}\n\n"
                        "was already processed automatically.\n\n"
                        "Do you want to open it in the manual editor?",
                        QMessageBox.Yes | QMessageBox.No,
                        QMessageBox.No
                    )

                    if reply == QMessageBox.Yes:
                        self.open_manual_from_failed_image(path)
                    continue
                ...
                if "manual" in existing_sources:
                    reply = QMessageBox.question(
                        self,
                        "Manual profile exists",
                        f"The image:\n\n{os.path.basename(path)}\n\n"
                        "already has a manual profile.\n\n"
                        "Do you still want to process it automatically?",
                        QMessageBox.Yes | QMessageBox.No,
                        QMessageBox.No
                    )
    ...
    def populate_details_table(self):
        ...
        for i, res in enumerate(self.results):
            ...
            # --- SOURCE / UNITS TAG ---
            source = res.get("source", "cv")
            units = res.get("units", "px")
            tag = f"{source.upper()}"
            tag_item = QTableWidgetItem(tag)

            # Optional: color-code
            if units == "cm":
                tag_item.setBackground(QColor("#e8f5e9"))  # light green
            else:
                tag_item.setBackground(QColor("#e3f2fd"))  # light blue

            self.details_table.setItem(i, 1, tag_item)

            self.details_table.setItem(i, 2, QTableWidgetItem(f"{res['rms']:.4f}"))
            self.details_table.setItem(i, 3, QTableWidgetItem(f"{res['cl']:.4f}"))
            self.details_table.setItem(i, 4, QTableWidgetItem(f"{res['h_max']:.4f}"))
            self.details_table.setItem(i, 5, QTableWidgetItem(f"{res['ic']:.2f}"))
            self.details_table.setItem(i, 6, QTableWidgetItem(f"{res['df']:.4f}"))
    ...
    def on_pins_modified(self, pins):
        ...
        r.update(metrics)
        self.update_ui(update_table_selection=False)
        self.update_table_row(self.current_idx)
    ...
    def update_table_row(self, row):
        ...
        self.details_table.setItem(row, 2, QTableWidgetItem(f"{r['rms']:.4f}"))
        self.details_table.setItem(row, 3, QTableWidgetItem(f"{r['cl']:.4f}"))
        self.details_table.setItem(row, 4, QTableWidgetItem(f"{r['h_max']:.4f}"))
        self.details_table.setItem(row, 5, QTableWidgetItem(f"{r['ic']:.2f}"))
        self.details_table.setItem(row, 6, QTableWidgetItem(f"{r['df']:.4f}"))
    ...
    def update_ui(self, update_table_selection=True):
        ...
        res = self.results[self.current_idx]
        # Manual profiles cannot be edited inline
        if res.get("source") == "manual":
            self.btn_edit_pins.setChecked(False)
            self.btn_edit_pins.setText("✏ Edit pins")
            self.btn_edit_pins.setToolTip("Edit pins")
        ...
        # =====================================================
        # CASE 1 — CV PIPELINE (PIXEL SPACE)
        # =====================================================
        if res.get("source", "cv") == "cv":
            img, pins = res['viz']
            viz = img.copy()
            ...
            for p in pins:
                c = (int(p['x']), int(p['y']))
                cv2.circle(viz, c, 4, (220, 210, 190), -1, cv2.LINE_AA)

            self.img_view.set_data(viz, pins, res.get('hy_calib'))

        # =====================================================
        # CASE 2 — MANUAL PIPELINE (PHYSICAL SPACE)
        # =====================================================
        else:
            # Prefer ROI image if available
            roi_img = res.get("roi_image")
